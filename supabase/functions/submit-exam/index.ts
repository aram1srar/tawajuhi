...
// Server-side answer key: question_id -> correctIndex
// Open-ended questions (correctIndex: -1) are excluded from scoring
const ANSWER_KEY: Record<string, number> = {
  ...
  b_pm1: 1, b_ba1: 0, b_mkt1: 1, b_fin1: 1, b_ops1: 0,
...
// Question type mapping for score categorization
...
serve(async (req) => {
  ...
  try {
    ...
    const { answers, openAnswers, careerPath, examType, durationSeconds, questionTimestamps } = await req.json();
    ...
    if (examType === "general") {
      // General exam: compute per-path scores
      ...
      for (const [qId, selectedIndex] of Object.entries(answers)) {
        ...
        if (correctIdx === undefined) continue; // skip unknown questions
      ...
      const { error: insertError } = await supabase.from("test_results").insert({
        ...
        answers: { ...answers, openAnswers: openAnswers || {} },
      ...
      // Pathway exam: compute theory vs interest scores
